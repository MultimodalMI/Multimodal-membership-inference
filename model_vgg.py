...
class EncoderCNN(nn.Module):
    def __init__(self, embed_size):
        ...
        self.linear = nn.Linear(in_features=25088, out_features=embed_size)
        self.bn = nn.BatchNorm1d(embed_size, momentum=0.01)
        # modules = list(vgg.children())[:-1]      # delete the last fc layer.
        # self.vgg = nn.Sequential(*modules)
        # self.linear = nn.Linear(vgg.fc.in_features, embed_size)
        # self.bn = nn.BatchNorm1d(embed_size, momentum=0.01)
...
class DecoderRNN(nn.Module):
    ...
    def sample(self, features, states=None):
        ...
        for i in range(self.max_seg_length):
            ...
            prob, _ = nn.functional.softmax(outputs,1).max(1)    # predicted: (batch_size)
